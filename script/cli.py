...
def main():
    ...
    async def run():
        ...
        if args.command == "extract":
            return extract_page(args.url, path.read_text()).model_dump()
        ctx = PageContext.model_validate_json(path.read_text())
        if args.command == "analyze":
            return (await analyze(ctx, settings, deep=args.deep)).model_dump()
