...
def evidence_for(ctx, layers):
    ...
    for i, layer in enumerate(layers):
        for j, signal in enumerate(layer.signals):
            evidence[f"E{i}_{j}"] = {"source": layer.layer, "signal_id": signal.id,
                "detail": redact(signal.detail), "grade": signal.grade, "weight": signal.weight}
...
async def structured(llm, schema, instructions, payload):
    ...
    response = await llm.request(model=llm.settings.model, reasoning={"effort": "low"},
...
async def assess(ctx, layers, llm):
    if not llm.settings.llm_enabled or not llm.settings.allow_content_upload or not llm.settings.api_key.get_secret_value():
    ...
    try:
        answer = await structured(llm, Assessment,
            ...
            {"evidence": evidence, "coverage": [{"layer": layer.layer, "status": layer.status} for layer in layers],
             "behavior": ctx.behavior})
        references = answer.action_evidence + [ref for h in answer.hypotheses for ref in h.supporting + h.opposing]
...
def apply_assessment(decision, report):
    ...
    if requested != "observe":
        ...
        strong = any(report["evidence"][ref].get("weight", 0) >= 15 and
                     report["evidence"][ref]["source"] not in {"VISION", "L9", "L16"}
...
def workflow_layer(report):
    return LayerResult(layer="WORKFLOW", status=report["status"],
        user_facing_reason=report.get("reason", "AI 已完成假設比較與證據整合"),
        feedback=(["網站可能目的：" + report["assessment"]["intent"],
                   *["假設：" + h["explanation"] + "；支持：" + ", ".join(h["supporting"]) + "；反證：" + ", ".join(h["opposing"]) for h in report["assessment"]["hypotheses"]],
...
class Diagnosis(Model):
    category: Literal["collection", "interpretation", "evidence_weight", "intervention_timing", "insufficient"]
...
async def diagnose(analysis, confirmed_label, llm):
    ...
    answer = await structured(llm, Diagnosis,
        ...
        {"confirmed_label": confirmed_label, "decision": analysis.get("decision"), "evidence": evidence,
         "coverage": [{"layer": layer["layer"], "status": layer["status"]} for layer in analysis.get("layers", [])]})
