...
try {
 ...
 await worker.evaluate(async()=>{
  await chrome.storage.local.set({settings:{pairingToken:'test-pairing-token-12345678901234567890',backendEnabled:false,llmEnabled:false}});
  globalThis.featureCalls=[];
  const original=globalThis.fetch;
  globalThis.fetch=async(input,init)=>{
   if(String(input).startsWith('http://127.0.0.1:8765/v1/')) {
    const body=JSON.parse(init.body);
    featureCalls.push({screenshot:!!body.screenshot,includeLLM:body.include_llm});
 ...
 await page.route('https://feature-test.example/**',route=>route.fulfill({contentType:'text/html',body:'<title>Feature integration</title><p>Plain website content.</p>'}));
