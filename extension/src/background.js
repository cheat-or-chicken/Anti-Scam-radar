...
async function analyzeScreenshot() {
  ...
  parseURL(tab.url);
  const snapshot = await chrome.tabs.sendMessage(tab.id, { type: "RADAR_CONTEXT" });
  if (!snapshot?.context) throw Error("PAGE_NOT_READY");
  const frame = await chrome.webNavigation.getFrame({ tabId: tab.id, frameId: 0 });
  if (!frame || frame.url !== tab.url) throw Error("STALE_PAGE");

  const ctx = sanitize(snapshot.context, tab.url);
  ...
  if (
    !latest ||
    latest.documentId !== frame.documentId ||
    latest.originalUrl !== tab.url ||
    !(await currentFrame(tab.id, frame.documentId, tab.url))
  )
    throw Error("STALE_PAGE");
